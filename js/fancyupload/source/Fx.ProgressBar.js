...
(function($){
	...
	Fx.ProgressBar = new Class({
		...
		initialize: function(element, options) {
			...
			this.element = $(element).getElement('.file-progress');
			this.parent(options);
		...
		set: function(to) {
			...
			this.element.setStyle('width', Math.round(to)+'%').title = Math.round(to) + '%';
			this.element.getElement('.file-progress-perc').set('text', Math.round(to)+'%');
			//this.element.tween("background-position", + Math.round(to)+ "px 0px");
