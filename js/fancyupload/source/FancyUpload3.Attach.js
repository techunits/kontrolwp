...
FancyUpload3.Attach.File = new Class({
	...
	render: function() {
		...
		this.ui.element_data = new Element('div', {'class':'file-data'});
		...
		this.ui.element_data.grab(this.ui.title);
		this.ui.element_data.grab(this.ui.size);
		this.ui.remove_file = new Element('div', {'class': 'remove-file'}).setStyle('display', 'none');
		
		this.ui.remove_file.addEvent('click', function() {
			if(confirm('Remove this upload?')) {
				this.remove();
			}
		}.bind(this));
		...
		this.ui.element.adopt(
			this.ui.remove_file,
			this.ui.element_data,
			this.ui.cancel
		).inject(this.base.list).highlight();
		
				
		var progress = new Element('div', {'class': 'file-progress'});
		var progress_bar_length = new Element('div', {'class': 'file-progress-length'});
		progress_bar_length.grab(progress);
		progress_bar_length.inject(this.ui.cancel, 'after');
		var progress_perc = new Element('div', {'class': 'file-progress-perc'});
		progress.grab(progress_perc);
		this.ui.progress = new Fx.ProgressBar(progress_bar_length, {
			fit: true
		}).set(0);
	...
	onComplete: function() {
		...
		this.ui.remove_file.setStyle('display', 'block');
		...
		if (this.ui.progress) this.ui.progress = this.ui.progress.cancel().container.destroy();
